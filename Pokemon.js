// =================================================================
// บอทตรวจเช็คสถานีวิทยุอัตโนมัติ (Pokemon Radio Checker Bot) v3.0
// ชื่อไฟล์: Pokemon.js
// =================================================================
...
const stationList = [
  ...
  { 
    ...
    // 🔥 ลิงก์ตรงของแท้ที่คุณแกะรอยเจอมาด้วยตัวเอง
    url: "https://atimeonline3.smartclick.co.th/green" 
  },
  { 
    name: "EFM 94", 
    url: "https://stream.atimeonline.com/efm" 
  }
];

async function checkStation(station) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 7000); // ตั้งเวลา Timeout 7 วินาที

  try {
    // ส่ง Request แบบขอเช็คเศษเสี้ยวไฟล์เสียง (Range) เพื่อความรวดเร็วและไม่แช่สายสตรีมมิ่ง
    const response = await fetch(station.url, {
      signal: controller.signal,
      headers: { 
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Pokemon-Checker/3.0',
        'Accept': '*/*',
        'Range': 'bytes=0-1024' 
      }
    });
    ...
    if (response.ok || response.status === 206) {
      ...
      // ตรวจสอบ Content-Type ให้ครอบคลุมระบบสตรีมมิ่งสด
